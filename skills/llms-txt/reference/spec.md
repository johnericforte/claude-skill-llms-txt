...
- Any non-link content inside an H2 section (prose, a stray heading). H2 sections are file lists only; move prose and non-page links to the body.
- An H2 link whose target is not a fetchable `http(s)` URL (`mailto:`, relative, `#`). The context tool fetches every H2 link, so these break it.
...
## Where non-page links belong

H2 sections list your own fetchable document pages, because `llms_txt2ctx` downloads every H2 link to build the context files. Contact details, `mailto:` addresses, and external links (booking, social profiles) go in the body markdown, which the spec allows for project detail. Keep the H2 sections to pages the tool can fetch.

## Context generation realities

- `llms_txt2ctx` fetches live URLs, so generate the context files against the deployed site, after the pages and their `.md` versions are live.
- The `llms.txt` links must point at the `.md` versions, or the tool inlines each page's raw HTML and the output balloons.
- The tool prints its parsed link list before the XML; drop everything before the first `<project` line.
- Install it in an isolated way (`pipx run`, `uvx`, or a venv); a bare `pip install` fails on a managed Python (PEP 668).

Serving the files, the discovery headers, and per-page `.md` generation depend on the host stack. See `stacks.md`.
